...
@app.get('/api/version')
def version(): return jsonify(version=ASSET_VERSION)
...
# A deployment starts fresh workers, each computing the same frontend digest.
_asset_hash=hashlib.sha256()
for _file in sorted((ROOT/'web').rglob('*')):
    if _file.is_file():
        _asset_hash.update(_file.relative_to(ROOT/'web').as_posix().encode())
        _asset_hash.update(_file.read_bytes())
ASSET_VERSION=_asset_hash.hexdigest()

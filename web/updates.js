// Reload published web updates only when doing so will not interrupt a ride or a draft.
(() => {
  ...
  async function checkUpdate() {
    ...
    checking=true;
    try {
      const response=await fetch('version.json?ts='+Date.now(),{cache:'no-store',signal:AbortSignal.timeout(8000)});
      if(!response.ok)return;
      const data=await response.json();
      if(typeof data.version!=='string' || !/^[a-f0-9]{7,64}$/i.test(data.version))return;
    ...
    } catch(_) { /* Offline/local preview: retain current page. */ }

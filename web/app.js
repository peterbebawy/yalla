...
async function api(path,body){
  ...
  if(window.YALLA_CONFIG?.backend==='firebase'){
    if(!window.YALLA_FIREBASE)throw new Error('Firebase لم يتم تحميله بعد. راجع إعدادات الاتصال.');
    return window.YALLA_FIREBASE.api(path,body);
  }
  let response;
...
renderLogin();
(async()=>{
  if(window.YALLA_CONFIG?.backend==='firebase' && window.YALLA_FIREBASE){
    await window.YALLA_FIREBASE.ready;
    if(window.YALLA_FIREBASE.currentUser()){
      token='firebase';sessionStorage.setItem('yalla-token',token);await refresh(true);
    } else { token='';sessionStorage.removeItem('yalla-token'); }
  } else if(token) await refresh(true);
})();
setInterval(()=>{if(!document.hidden && (demo||token))refresh();},3000);
